...
describe("CoinFlip Contract", function () {
  describe("Deployment", function () {
    ...
    it("Should set the initial contract balance", async function () {
      ...
      expect(await coinFlip.getContractBalance()).to.equal(
  ...
  describe("Bet Function", function () {
    it("Should allow a user to place a bet", async function () {
      ...
      await coinFlip
        .connect(addr1)
        .bet(0, { value: ethers.parseEther("0.01") });
    ...
    it("Should revert if bet amount is too big", async function () {
      ...
      await expect(
        coinFlip.connect(addr1).bet(0, { value: ethers.parseEther("1") }),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__AmountTooBig");
    });
    ...
    it("Should revert if bet choice is invalid", async function () {
      ...
      await expect(
        coinFlip.connect(addr1).bet(2, { value: ethers.parseEther("0.01") }),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__InvalidBetChoice");
    });
    ...
    it("Should revert if a bet is already ongoing", async function () {
      ...
      await coinFlip
        ...
        .bet(0, { value: ethers.parseEther("0.01") });
      await expect(
        coinFlip.connect(addr1).bet(0, { value: ethers.parseEther("0.01") }),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__BetAlreadyOngoing");
    });
  });

  describe("Withdraw Function", function () {
    it("Should allow a user to withdraw their balance", async function () {
      const { coinFlip, coinFlipAddress, coordinator, addr1 } =
        await loadFixture(deployFixture);

      // Place a bet
      await coinFlip
        .connect(addr1)
        .bet(1, { value: ethers.parseEther("0.01") });
      ...
      await expect(() =>
        coinFlip.connect(addr1).withdrawPlayerBalance(),
      ).to.changeEtherBalance(addr1, ethers.parseEther("0.02"));
    ...
    it("Should revert if there are no funds to withdraw", async function () {
      ...
      await expect(
        coinFlip.connect(addr2).withdrawPlayerBalance(),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__NoFundsToWithdraw");
    });
    ...
    it("Should revert if a bet is ongoing", async function () {
      ...
      await expect(
        coinFlip.connect(addr1).withdrawPlayerBalance(),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__BetOngoing");
    });
  });

  describe("Deposit Function", function () {
    it("Should allow a user to deposit ETH", async function () {
      const { coinFlip, coinFlipAddress, addr1 } =
        await loadFixture(deployFixture);

      const depositAmount = ethers.parseEther("0.1");

      const contractBalanceBefore =
        await ethers.provider.getBalance(coinFlipAddress);

      await coinFlip.connect(addr1).deposit({ value: depositAmount });
      expect(await coinFlip.getContractBalance()).to.equal(
        depositAmount + contractBalanceBefore,
      );
    });
    ...
    it("Should revert if deposit amount is zero", async function () {
      ...
      await expect(
        coinFlip.connect(addr1).deposit({ value: ethers.parseEther("0") }),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__InsuffisantAmount");
    });
  });
  ...
  describe("Owner Functions", function () {
    it("Should allow the owner to withdraw contract balance", async function () {
      const { coinFlip, coinFlipAddress, owner } =
        await loadFixture(deployFixture);
    ...
    it("Should revert if there are no funds to withdraw", async function () {
      ...
      await coinFlip.connect(owner).withdrawContractBalance();
      await expect(
        coinFlip.connect(owner).withdrawContractBalance(),
      ).to.be.revertedWithCustomError(coinFlip, "CoinFlip__NoFundsToWithdraw");
    });
    ...
    it("Should revert if not owner", async function () {
      ...
      await expect(
        coinFlip.connect(addr1).withdrawContractBalance(),
